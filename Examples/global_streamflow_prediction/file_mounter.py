...
with open('/app/fuse/file_mount.json') as f:
    try:
        ...
        for cfg in file_config:
            ...
            os.makedirs(resource_dir + "/south_asia-mainland/20181112.00")
            shutil.chown(resource_dir, "root")
            os.chmod(fuse_cfg_file, 700)
            os.makedirs(cfg["mappedDirectory"] + "/south_asia-mainland/20181112.0")
            ...
            copy_cmd = "azcopy --quiet --source https://" + cfg["accountName"] + ".file.core.windows.net/" + resource_dir + "/south_asia-mainland/20181112.00 --destination " + cfg["mappedDirectory"] + "/south_asia-mainland/20181112.0 --source-key " + cfg["accountKey"] + " --recursive --exclude-older --exclude-newer"
